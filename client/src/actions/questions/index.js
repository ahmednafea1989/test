import {GET_QUESTIONS ,
     GET_QUESTIONS_ERROR,
        ADD_QUESTION ,
         ADD_QUESTION_ERROR,ADD_SCORE,ADD_SCORE_ERROR} from '../types';
import axios from 'axios';
export const getQuestions = () => async dispatch => {
    try {
      const { data } = await axios.get('/api/quiz',{ headers: { 'authorization': localStorage.getItem('token')} });
      dispatch({ type: GET_QUESTIONS , payload: data });
      console.log(data);
      
    } catch (e) {
      dispatch({ type: GET_QUESTIONS_ERROR, payload: 'Something went wrong, please refresh the page to try again'})
    }
  };
